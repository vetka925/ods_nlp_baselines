...
class AutoLinearModel:
    ...
    def vectorizers_prepare(self):
        ...
        supervised_stacked = SupervisedStackedVectorizer(
            count_char, count_word, self.y_train
        )
        supervised_stacked.fit(self.X_train)

        self.vectorizers["tfidf_ngram_level"] = tfidf_ngram
        self.vectorizers["tfidf_word_level"] = tfidf_word
        self.vectorizers["tfidf_stacked"] = StackedVectorizer(tfidf_char, tfidf_word)
        self.vectorizers["supervised_ngram_level"] = supervised_ngram
        self.vectorizers["supervised_word_level"] = supervised_word
        self.vectorizers["supervised_stacked"] = supervised_stacked
        ...
        for vec_name in self.vectorizers:
            ...
            for param in self.params:
                ...
                for score in self.scoring_func_dict:
                    result[score].append(
                        self.scoring_func_dict[score](self.y_test, preds)
                    )
    ...
    def best_classifier_vectorizer(self, scoring_func_name):
        return self.report.iloc[self.report.scoring_func_name.argmax()].to_dict()
...
class BertFinetuner(pl.LightningModule):
    def __init__(self, bert_model, train_dataloader, val_dataloader, test_dataloader=None, scoring_func=f1_score):
        ...
        self.classifier = torch.nn.Sequential(torch.nn.Linear(bert.config.hidden_size, 2),
